...
export default function AttendanceReportsPage() {
  ...
  const { data: reports = [], mutate } = useSWR<ReportBase[]>(
    "attendanceReports",
    reportService.getAttendanceReports
  );
                  ...
                  <tr key={r.id}>
                    ...
                    <td className="flex gap-2.5">
                      <button
                        onClick={async () => {
                          const res =
                            await reportService.downloadAttendanceReport(r.id);
                          // utils для скачивания blob
                          const url = URL.createObjectURL(res.data);
                          const a = document.createElement("a");
                          a.href = url;
                          a.download = `attendance_${r.id}.xlsx`;
                          a.click();
                      ...
                      <button
                        onClick={async () => {
                          await reportService.deleteReport("attendance", r.id);
                          mutate();
                        }}
