    ...
    <ProtectedRoute>
      <Layout>
        ...
        <div className="tables board">
          <TableHeadline title="Отчёты по сотруднику" btn={false} />
          ...
          <div className="overflow-x-auto">
            <table>
              ...
              <tbody>
                {paged.map((r) => (
                  <tr key={r.id}>
                    <td>{/* сюда подставить имя из payload */}</td>
                    <td>{r.date_from}</td>
                    <td>{r.date_to}</td>
                    ...
                    <td className="flex gap-2.5">
                      <button
                        onClick={async () => {
                          const res =
                            await reportService.downloadEmployeeReport(r.id);
                          const url = URL.createObjectURL(res.data);
                          const a = document.createElement("a");
                          a.href = url;
                          a.download = `employee_${r.id}.pdf`;
                          a.click();
                      ...
                      <button
                        onClick={async () => {
                          await reportService.deleteReport("employee", r.id);
                          mutate();
                        }}
