...
import DepartmentReportModal from "@/components/reports/DepartmentReportModal";
    ...
    <ProtectedRoute>
      <Layout>
        ...
        <div className="tables board">
          <TableHeadline title="Отчёты по отделам" btn={false} />
                  ...
                  <tr key={r.id}>
                    ...
                    <td className="flex gap-2.5">
                      <button
                        onClick={async () => {
                          const res =
                            await reportService.downloadDepartmentReport(r.id, {
                              department: r.id,
                            });
                          const url = URL.createObjectURL(res.data);
                          const a = document.createElement("a");
                          a.href = url;
                          a.download = `department_${r.id}.xlsx`;
                          a.click();
                      ...
                      <button
                        onClick={async () => {
                          await reportService.deleteReport("department", r.id);
                          mutate();
                        }}
