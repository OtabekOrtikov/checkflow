...
import GenderReportModal from "@/components/reports/GenderReportModal";
import { PageHeadline } from "@/components/layout/PageHeadline";
import { Footer } from "@/components/Footer";

export default function GenderReportsPage() {
  const [page, setPage] = useState(1);
  const pageSize = 10;

  const { data: reports = [], mutate } = useSWR<ReportBase[]>(
    "genderReports",
    reportService.getGenderReports
  );
    ...
    <ProtectedRoute>
      <Layout>
        ...
        <div className="tables board">
          <TableHeadline title="Отчёты по гендеру" btn={false} />
                ...
                {paged.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="flex gap-2.5">
                      <button
                        onClick={async () => {
                          const res = await reportService.downloadGenderReport(
                            r.id,
                            {
                              gender: filters.gender, // подставляем реальный фильтр
                            }
                          );
                          const url = URL.createObjectURL(res.data);
                          const a = document.createElement("a");
                          a.href = url;
                          a.download = `gender_${r.id}.xlsx`;
                          a.click();
                      ...
                      <button
                        onClick={async () => {
                          await reportService.deleteReport("gender", r.id);
                          mutate();
                        }}
