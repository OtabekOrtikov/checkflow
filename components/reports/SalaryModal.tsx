// src/components/reports/SalaryModal.tsx
"use client";
...
export default function SalaryModal({ open, onClose, onSaved }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    await reportService.createSalaryReport(form);
    setLoading(false);
    onSaved();
  };
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          ...
          >
            <Dialog.Panel className="w-full max-w-3xl bg-white p-6 space-y-4 rounded-2xl">
              <ModalTitle title="Отчёт по зарплате и штрафам" onClose={onClose} />
