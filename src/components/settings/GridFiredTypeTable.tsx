...
interface Props {
  ...
  onAdd: () => void;
  onEdit: (item: DismissalType) => void;
  onDelete: (id: string) => void;
}

export const GridFiredTypeTable: React.FC<Props> = ({
  data, loading, page, pageSize, setPage, onAdd, onEdit, onDelete,
}) => {
          ...
          paged.map((row) => (
            ...
            >
              ...
              <div className="flex justify-start">
                <div className="h-4 w-10 rounded-full" style={{ backgroundColor: row.color }} />
              </div>
              <div>{row.description}</div>
              <div className="flex justify-center gap-2">
                <button onClick={() => onEdit(row)} className="px-3 py-1 border rounded-full text-(--primary) bg-(--white)">
                  <PencilIcon className="w-[16px] h-[16px]" />
                </button>
                <button onClick={() => onDelete(row.id)} className="px-3 py-1 border rounded-full text-(--red) bg-(--white)">
